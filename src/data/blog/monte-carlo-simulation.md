---
title: プロップファームのEA合格率をモンテカルロシミュレーションで予測する方法
author: はちまき
pubDatetime: 2026-03-07T17:30:00.000+09:00
modDatetime:
...
モンテカルロシミュレーション（MC）は、同じ統計的特性（勝率・リスクリワード比（平均利益÷平均損失）・トレード頻度）を持つランダムなトレード系列を **数万回生成** し、結果の分布を調べる手法です。
...
1回のシミュレーション:
  口座残高 = 初期資金
  for 日数 in 200日:
    その日のトレード数をランダム生成（ポアソン分布：まれな事象の発生回数を表す確率分布）
