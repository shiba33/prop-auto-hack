---
title: EA運用中のドローダウン対処法｜連敗が「正常」か「異常」かの統計的判断基準
author: はちまき
...
tags:
  ...
  - risk-management
  - backtest
description: EAの連敗やドローダウンが「想定内」か「異常」かを統計的に判断する方法。プロップファーム運用で失格を防ぐための基準値を公開。
---
...
| ケース | 回復までの日数 | 発生確率 |
|--------|---------------|---------|
| 普通の運 | 約18日（2〜3週間） | 50%のケースで18日以内に回復 |
| 運が悪いとき | 最大72日（約2ヶ月半） | 5%の確率でここまで長引く |

つまり3%のDDは **普通なら2〜3週間で回復** しますが、運が悪いと **2ヶ月以上かかる** こともあります。72日間回復しなくても、それは20回に1回は起きうる「正常な範囲」です。
...
自己制限2%であれば、デイリーDD 5%制限までに2.5倍の余裕があります。この余裕がスプレッド（売買コスト）拡大やスリッページ（注文価格と約定価格のずれ）などの不測の事態を吸収します。
...
一般的に、 **プロップファームで失格するトレーダーの多くは、戦略の問題ではなく心理・規律の問題で脱落している** と言われています。
...
- ストップロス（損切りライン）を設定しないトレーダーは、大きな損失で一発失格しやすい
- DD中にロット（取引量）を増やす「リベンジトレード」は、失格を早める典型的な行動
- トレード日記（感情記録含む）をつけると、冷静な判断を維持しやすい
