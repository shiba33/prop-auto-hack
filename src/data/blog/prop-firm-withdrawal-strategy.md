...
インスタントプラン（Instant Funding）は、チャレンジなしで即座にFunded（本番運用）アカウントが付与されるプランです。
...
インスタントプランでは出金のタイミング1つで生存率が大きく変わります。モンテカルロ分析（ランダムなトレード結果を大量にシミュレーションする手法）を3万回実施した結果、出金頻度による差が明確に出ました。
