...
レバレッジ（てこの原理で少額の資金で大きな取引ができる仕組み）が低いと、1ポジション（保有中のトレード）あたりに必要な証拠金が大きくなります。1ポジションだけなら問題になりにくいですが、 **複数戦略を同時に動かして複数ポジションを持つ場合、証拠金が足りなくなる** リスクがあります。

例えば$100K口座でEURUSD（ユーロ/米ドル）を1ロット（10万通貨の取引単位）持つ場合、レバレッジ1:100なら証拠金は約$1,000ですが、1:30では約$3,333必要です。3戦略が同時にポジションを持つと証拠金だけで$10,000を超え、DD余裕が圧迫されます。
