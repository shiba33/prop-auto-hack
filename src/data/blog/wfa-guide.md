---
title: WFA（ウォークフォワード分析）入門｜EA開発でバックテストの嘘を見抜く方法
author: はちまき
...
ISでProfit Factor（総利益÷総損失の比率。1.0以上で利益が出ている）1.79を記録した設定が、OOSでは最悪の結果になることがありました。ISの数字だけで判断するのは危険です。
...
WFAを通過していないEAでチャレンジに挑むのは、耐震検査なしで建物を使い始めるようなものです。地味な作業ですが、チャレンジ費用を守るための最低限の品質保証がWFAです。
