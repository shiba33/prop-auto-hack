...
この制約のため、VPS（仮想専用サーバー：24時間稼働するレンタルサーバー）での常時稼働＋定期的な監視・再起動のスクリプトが必要になります。VPSはWindows環境を推奨します。Ubuntu上のWineでも動作はしますが、cTraderとの相性問題が発生しやすく、安定性に欠けます。
...
- **約定品質が高い** — NDD方式（ブローカーが注文を直接市場に流す仕組み）で透明性がある
- **板情報が見える** — デプスオブマーケット（各価格帯の注文量を表示する機能）で流動性を確認できる
